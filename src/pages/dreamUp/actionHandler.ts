...
const useCustomHook = () => {
  ...
  const [lifeAssessment, setLifeAssessment] =
    useRecoilState(lifeAssessmentState);
  ...
  const getBarsData = async () => {
    ...
    if (data?.response && data?.response?.length !== 0) {
      const goals = data?.response;
      completedGoal = goals?.filter((g: any) => {
    ...
    const mGoal = {
      ...
      progressbarValue:
        mainGoal[0]?.totalTasks !== 0
          ? (
              (mainGoal[0]?.completedTasks / mainGoal[0]?.totalTasks) *
              100
            ).toFixed(0)
          : 0,
      storage: '128GB',
    ...
    const last = {
      content:
        completedGoal[0]?.totalTasks !== 0
          ? `${completedGoal[0]?.completedTasks} of ${completedGoal[0]?.totalTasks} tasks completed`
          : 'No task available',
      icon: AchivmentIcon,
      ...
      progressbarValue:
        completedGoal[0]?.totalTasks !== 0
          ? (
              (completedGoal[0]?.completedTasks /
                completedGoal[0]?.totalTasks) *
              100
            ).toFixed(0)
          : 0,
      storage: '128GB',
