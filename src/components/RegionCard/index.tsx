...
export const RegionCard: FC<{ regionData: IRegionData }> = (props) => {
  const { regionData }: any = props;
  const regionDataList = Object.keys(regionData);
  return (
    <div className="">
      <p className="font-semibold text-[20px] leading-[28px] pb-[32px]">Region</p>
      {regionDataList?.map((item: string) => (
