...
export const Alert: React.FC = ({ title, type, width, showHide, icon, cancelBtntxt, okBtntxt, children }: any) => {
    ...
    return (
        <>
            <Modal
                ...
                footer={[
                    <Button onClick={() => { setOpen(!open) }} key="Cancel" style={{ border: `1px solid ${alertColor}`, color: alertColor, padding: '0px 20px' }}>
                        {cancelBtntxt}
                    </Button>,
                    <Button onClick={() => { setOpen(!open) }} key="submit" style={{ backgroundColor: alertColor, color: '#fff', border: `1px solid ${alertColor}`, padding: '0px 20px' }}>
                        {okBtntxt}
                    </Button>,
                ]}
            >
